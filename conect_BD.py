...
class Conect_bd:
    ...
    def v2_connection_rj_detalhado(id):
        ...
        #Criando o DataFrame:
        rj_df_subtotais =""
        rj_df_subtotais = pd.DataFrame({"Descrição": descricoes,"Valor": valores})
        # Nomeando as colunas
        rj_df_subtotais.columns = ["Descrição", "Valor"] 
        
        with pd.ExcelWriter('arquivos\IW_PROD_RJ_Resultado.xlsx', engine='openpyxl', mode='w') as writer:
            data_frame.to_excel(writer, sheet_name='Analitico', index=False, header=True)
            rj_df_subtotais.to_excel(writer, sheet_name='Sintetico', index=False, header=False)
        return rj_df_subtotais
    ...
    def v2_connection_sp_detalhado(id):
        ...
        #Criando o DataFrame:
        sp_df_subtotais =""
        sp_df_subtotais = pd.DataFrame({"Descrição": descricoes,"Valor": valores})
        # Nomeando as colunas
        sp_df_subtotais.columns = ["Descrição", "Valor"] 
        
        with pd.ExcelWriter('arquivos\IW_PROD_SP_Resultado.xlsx', engine='openpyxl', mode='w') as writer:
            data_frame.to_excel(writer, sheet_name='Analitico', index=False, header=True)
            sp_df_subtotais.to_excel(writer, sheet_name='Sintetico', index=False, header=False)
        return sp_df_subtotais